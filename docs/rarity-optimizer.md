...
3. ✅ **Optimizer B shipped (resource-constrained, max-keepers).** "Batch upgrade advisor" in the Tool
   ...
   - **Cost-per-keeper + culling (the "don't upgrade the Commons" rule).** Each below-target band shows
     `remaining upgrade mats ÷ its keeper chance Pk(t,r)` and is flagged **leave** when that exceeds a
     fresh tool's cost-per-keeper (`cumCost(maxT) ÷ Pk(1,Common)`), or when it can't reach the target in
     time (`Pk < 0.5%`). Reproduces the folklore: T2 Common ≈ 25k/keeper (leave) vs T2 Uncommon ≈ 10k
     (keep), fresh ≈ 18k. The sim **acts on it** — leave bands are culled (not carried to the next tier),
     so e.g. the 70 T2 Commons don't reappear at T3. Summary shows keepers + left/culled + mats saved
     (needs live mat prices; without them nothing is culled except can't-reach bands).
